...
logger = logging.getLogger("TestDailyAirAPIScraper")

def main():
    """測試德安航空API爬蟲，並打印結果"""
    try:
        logger.info("開始測試德安航空API爬蟲...")
        ...
        # 獲取資料來源
        sources = set(flight.get('source', 'unknown') for flight in flights)
        logger.info(f"航班資料來源: {', '.join(sources)}")
        
        # 檢查儲存的JSON文件
        output_dir = "flight_data"
        output_file = os.path.join(output_dir, f"dailyair_api_flights_{tomorrow}.json")
        ...
        if os.path.exists(debug_dir):
            api_sample_files = [f for f in os.listdir(debug_dir) if f.startswith("dailyair_api_")]
            logger.info(f"找到 {len(api_sample_files)} 個API樣本響應文件")
            
            # 打印前5個樣本文件的信息
            for i, filename in enumerate(api_sample_files[:5]):
                file_path = os.path.join(debug_dir, filename)
                file_size = os.path.getsize(file_path)
                logger.info(f"  - 樣本文件 {i+1}: {filename} (大小: {file_size} 字節)")
